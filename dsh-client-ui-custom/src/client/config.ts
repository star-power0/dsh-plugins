...
export interface CustomThemeConfig {
   ...
   * The ladder is what every neutral text in the UI rides (census: label-primary
   * ×177, tertiary ×171, secondary ×142, caption ×55, dimmed ×17): primary =
   * 聊天正文/侧栏会话标题/面板标题, secondary = 副标题/元信息/任务步骤,
   * tertiary = 时间戳/弱说明/上下文注入文字, caption = 最小说明文字,
   * dimmed = 更浅的辅助文字. The `dark*` variants override dark mode only;
   * '' falls back to the light value (or stock when that is unset too).
   */
  inkPrimary: string
  inkSecondary: string
  inkTertiary: string
  inkCaption: string
  inkDimmed: string
  darkInkPrimary: string
  darkInkSecondary: string
  darkInkTertiary: string
  darkInkCaption: string
  darkInkDimmed: string
...
export function normalizeConfig(
  ...
  return {
    ...
    // Text-ink overrides: trimmed CSS colors; '' keeps the stock ladder step.
    inkPrimary: cleanString(merged.inkPrimary, DEFAULTS.inkPrimary),
    inkSecondary: cleanString(merged.inkSecondary, DEFAULTS.inkSecondary),
    inkTertiary: cleanString(merged.inkTertiary, DEFAULTS.inkTertiary),
    inkCaption: cleanString(merged.inkCaption, DEFAULTS.inkCaption),
    inkDimmed: cleanString(merged.inkDimmed, DEFAULTS.inkDimmed),
    darkInkPrimary: cleanString(merged.darkInkPrimary, DEFAULTS.darkInkPrimary),
    darkInkSecondary: cleanString(merged.darkInkSecondary, DEFAULTS.darkInkSecondary),
    darkInkTertiary: cleanString(merged.darkInkTertiary, DEFAULTS.darkInkTertiary),
    darkInkCaption: cleanString(merged.darkInkCaption, DEFAULTS.darkInkCaption),
    darkInkDimmed: cleanString(merged.darkInkDimmed, DEFAULTS.darkInkDimmed),
...
export const CONFIG_KEYS: readonly (keyof CustomThemeConfig)[] = [
  ...
  'focusGlow', 'wallpaperTone', 'darkAccent',
  'inkPrimary', 'inkSecondary', 'inkTertiary', 'inkCaption', 'inkDimmed',
  'darkInkPrimary', 'darkInkSecondary', 'darkInkTertiary', 'darkInkCaption', 'darkInkDimmed',
  'customCss', 'customVars',
  'shortcuts',
]
