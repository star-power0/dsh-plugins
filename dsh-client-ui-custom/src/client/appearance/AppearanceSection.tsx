...
type InkLabel = 'inkPrimary' | 'inkSecondary' | 'inkTertiary' | 'inkCaption' | 'inkDimmed'

/** The official neutral label ladder, exposed as text-ink knobs. Stocks are the
 * frontend's own ladder values so a color picker always starts from the color
 * actually in effect (light: primary/secondary/tertiary/caption/dimmed). */
const INK_STEPS: readonly { field: ThemeField; darkField: ThemeField; label: InkLabel; lightStock: string; darkStock: string }[] = [
  { field: 'inkPrimary', darkField: 'darkInkPrimary', label: 'inkPrimary', lightStock: '#0f1115', darkStock: '#f9fafb' },
  { field: 'inkSecondary', darkField: 'darkInkSecondary', label: 'inkSecondary', lightStock: '#61666b', darkStock: '#cfd3d6' },
  { field: 'inkTertiary', darkField: 'darkInkTertiary', label: 'inkTertiary', lightStock: '#81858c', darkStock: '#adb2b8' },
  { field: 'inkCaption', darkField: 'darkInkCaption', label: 'inkCaption', lightStock: '#adb2b8', darkStock: '#81858c' },
  { field: 'inkDimmed', darkField: 'darkInkDimmed', label: 'inkDimmed', lightStock: '#e1e5ee', darkStock: '#43454a' },
]
...
export function AppearanceSection({
  ...
  return (
    <div className={css.section}>
      ...
      {/* 文字颜色: the neutral label ladder, per mode (empty = stock / follow). */}
      <GroupCard title={translator('groupInk')} resetLabel={translator('groupReset')} group="ink" writable={state.writable} onReset={resetGroup}>
        <p className={css.hint}>{translator('inkHint')}</p>
        <div className={css.subHeading}>{translator('inkLightHeading')}</div>
        {INK_STEPS.map((step) => (
          <div key={step.field} className={css.row}>
            <label className={css.label} htmlFor={`appearance-${step.field}`}>{translator(step.label)}</label>
            <span className={css.slider}>
              <input
                id={`appearance-${step.field}`}
                className={css.color}
                type="color"
                value={str(step.field, '') || step.lightStock}
                disabled={!state.writable}
                onChange={(event) => setField(step.field, event.target.value)}
              />
              <input
                className={css.text}
                type="text"
                value={str(step.field, '')}
                placeholder={translator('inkStockPlaceholder')}
                title={step.lightStock}
                disabled={!state.writable}
                onChange={(event) => setField(step.field, event.target.value)}
              />
            </span>
        ...
        <div className={css.subHeading}>{translator('inkDarkHeading')}</div>
        {INK_STEPS.map((step) => (
          <div key={step.darkField} className={css.row}>
            <label className={css.label} htmlFor={`appearance-${step.darkField}`}>{translator(step.label)}</label>
            <span className={css.slider}>
              <input
                id={`appearance-${step.darkField}`}
                className={css.color}
                type="color"
                value={str(step.darkField, '') || str(step.field, '') || step.darkStock}
                disabled={!state.writable}
                onChange={(event) => setField(step.darkField, event.target.value)}
              />
              <input
                className={css.text}
                type="text"
                value={str(step.darkField, '')}
                placeholder={translator('inkFollowLight')}
                title={step.darkStock}
                disabled={!state.writable}
                onChange={(event) => setField(step.darkField, event.target.value)}
              />
            </span>
