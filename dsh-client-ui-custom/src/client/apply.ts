...
const isNeutralConfig = (config: CustomThemeConfig): boolean =>
  ...
  && config.inkPrimary === DEFAULTS.inkPrimary
  && config.inkSecondary === DEFAULTS.inkSecondary
  && config.inkTertiary === DEFAULTS.inkTertiary
  && config.inkCaption === DEFAULTS.inkCaption
  && config.inkDimmed === DEFAULTS.inkDimmed
  && config.darkInkPrimary === DEFAULTS.darkInkPrimary
  && config.darkInkSecondary === DEFAULTS.darkInkSecondary
  && config.darkInkTertiary === DEFAULTS.darkInkTertiary
  && config.darkInkCaption === DEFAULTS.darkInkCaption
  && config.darkInkDimmed === DEFAULTS.darkInkDimmed
...
export function applyConfig(config: CustomThemeConfig): void {
  ...
  // Text-ink overrides over the official neutral label ladder: only written
  // when the user picked a color ('' removes the property so the stylesheet
  // falls back to the stock ladder step). The dark variants ride the same
  // pattern; the stylesheet chains dark → light → stock.
  const inkVars: readonly (readonly [keyof CustomThemeConfig, string])[] = [
    ['inkPrimary', '--dsu-ink-primary'],
    ['inkSecondary', '--dsu-ink-secondary'],
    ['inkTertiary', '--dsu-ink-tertiary'],
    ['inkCaption', '--dsu-ink-caption'],
    ['inkDimmed', '--dsu-ink-dimmed'],
    ['darkInkPrimary', '--dsu-dark-ink-primary'],
    ['darkInkSecondary', '--dsu-dark-ink-secondary'],
    ['darkInkTertiary', '--dsu-dark-ink-tertiary'],
    ['darkInkCaption', '--dsu-dark-ink-caption'],
    ['darkInkDimmed', '--dsu-dark-ink-dimmed'],
  ]
  for (const [field, name] of inkVars) {
    const value = config[field]
    if (typeof value === 'string' && value !== '') set(name, value)
