...
- 文字颜色旋钮（`ink*` / `darkInk*`）直接覆盖官方中性文字色阶
  `--dsw-alias-label-{primary,secondary,tertiary,caption,dimmed}`，未设置的
  档位回退到官方原值；深色模式按 `darkInk* → ink* → 官方深色值` 链条取值。
  旧的「聊天列不透明度」旋钮已移除：其 token `--dsw-chat-surface` 在当前
  官方前端已无任何消费方，属于死旋钮。
- 框架结构：
...
| `inkPrimary` | string | `''` | Primary text color (chat body / sidebar session titles / panel titles); empty = default |
| `inkSecondary` | string | `''` | Secondary text color (subtitles / list metadata / task steps); empty = default |
| `inkTertiary` | string | `''` | Tertiary text color (timestamps / soft hints / context-injection text); empty = default |
| `inkCaption` | string | `''` | Caption text color (smallest hints); empty = default |
| `inkDimmed` | string | `''` | Dimmed text color (fainter auxiliary text); empty = default |
| `darkInkPrimary` … `darkInkDimmed` | string | `''` | Dark-mode overrides of the five steps above; empty = follow the light value |
