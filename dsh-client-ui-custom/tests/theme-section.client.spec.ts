...
describe('configFromThemeSection', () => {
  ...
  it('re-resolves wallpaper blur when the runtime glass level changes', () => {
    const light = configFromThemeSection(normalized, { glass: 'light', wallpaper: undefined })
    const off = configFromThemeSection(normalized, { glass: 'off', wallpaper: undefined })
    const mica = configFromThemeSection(normalized, { glass: 'mica', wallpaper: undefined })
    expect(light.wallpaperBlur).toBe(6)
    expect(off.wallpaperBlur).toBe(0)
    expect(mica.wallpaperBlur).toBe(22)
  })

  it('keeps a loader-level explicit wallpaper blur while its glass level is unchanged', () => {
    const custom = normalizeConfig({ glass: 'frosted', wallpaperBlur: 8 }, undefined)
    const same = configFromThemeSection(custom, { glass: 'frosted', wallpaper: undefined })
    const changed = configFromThemeSection(custom, { glass: 'mica', wallpaper: undefined })
    expect(same.wallpaperBlur).toBe(8)
    expect(changed.wallpaperBlur).toBe(22)
  })
  ...
  it('rejects an invalid glass level (falls back to the loader)', () => {
    const config = configFromThemeSection(normalized, {
      ...
      inkPrimary: undefined, inputOpacity: undefined, codeBlockOpacity: undefined,
  ...
  it('never lands an explicit undefined on the optional darkSurfaceOpacity', () => {
    const config = configFromThemeSection(normalized, {
      ...
      surfaceOpacity: undefined, sidebarOpacity: undefined, inkPrimary: undefined,
  ...
  it('treats empty string knobs as "no override" (falls back to the loader)', () => {
    ...
    const config = configFromThemeSection(normalized, {
      ...
      surfaceOpacity: undefined, sidebarOpacity: undefined, inkPrimary: '',
      inkSecondary: undefined, darkInkPrimary: undefined,
      inputOpacity: undefined, codeBlockOpacity: undefined, darkSurfaceOpacity: undefined,
    ...
    expect(config.inkPrimary).toBe(normalized.inkPrimary)
  })
  ...
  it('inherits the dark surface opacity from the live surfaceOpacity when unset', () => {
    ...
    const config = configFromThemeSection(normalized, {
      ...
      surfaceOpacity: 72, sidebarOpacity: undefined, inkPrimary: undefined,
      inputOpacity: undefined, codeBlockOpacity: undefined, darkSurfaceOpacity: undefined,
      gradient: undefined, darkScrim: undefined, fontFamily: undefined,
      scrollbarAccent: undefined, vignette: undefined,
    })
