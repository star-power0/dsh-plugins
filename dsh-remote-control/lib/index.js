...
*/
/** 配对码有效期（毫秒）。 */
const PAIR_CODE_TTL_MS = 600 * 1e3;
...
var PairingStore = class {
	...
	/** 生成一条新配对码。 */
	createPairCode() {
		const code = String(randomBytes(4).readUInt32BE(0) % 1e8).padStart(8, "0");
		this.pendingCodes.set(code, { expiresAt: Date.now() + PAIR_CODE_TTL_MS });
		const now = Date.now();
		for (const [key, pending] of this.pendingCodes) if (pending.expiresAt <= now) this.pendingCodes.delete(key);
		return {
			code,
	...
	async consumePairCode(code, deviceName) {
		...
		if (pending === void 0 || pending.expiresAt <= Date.now()) return null;
...
function createProxyBridge(apiProxy) {
	...
	async function call(method, payload) {
		...
		const envelope = await (await handler.fetch(request)).json();
		if (envelope.type !== "server-response" || envelope.result === void 0) return {
			ok: false,
			error: {
				code: "internal",
				message: "网关返回了未知信封"
...
function renderPairPage(prefilledCode, errorText) {
	return `${BASE_HEAD}
...
function collectLocalHostnames() {
	...
	for (const list of Object.values(networkInterfaces())) for (const entry of list ?? []) {
		...
		if (entry.family === "IPv4") names.add(entry.address);
		else names.add(`[${entry.address}]`);
	}
	return [...names];
}
/** 解析 Cookie 头为键值对。 */
	...
	allow(ip) {
		...
		if (bucket === void 0 || bucket.resetAt <= now) {
			...
			if (this.buckets.size > 1024) {
				for (const [key, entry] of this.buckets) if (entry.resetAt <= now) this.buckets.delete(key);
			}
			return true;
		}
		bucket.count += 1;
...
async function startGateway(deps) {
	...
	for (const host of config.trustedHosts) localHostnames.add(host);
	...
	async function handle(req, res) {
		...
		let hostName = "";
		if (typeof hostHeader === "string") try {
			hostName = new URL(`http://${hostHeader}`).hostname;
		} catch {
			hostName = "";
		}
		if (!(hostName !== "" && (localHostnames.has(hostName) || localHostnames.has(`[${hostName}]`)))) {
			json$1(res, 403, {
				ok: false,
				error: {
					code: "forbidden",
		...
		if (req.method === "POST" && rawPath === "/remote/pair") {
			...
			const name = typeof body?.deviceName === "string" ? body.deviceName : "";
			if (code === "") {
				json$1(res, 400, {
					ok: false,
					error: {
		...
		if (req.method === "POST" && rawPath === "/remote/respond") {
			...
			try {
				json$1(res, 200, await proxy.respond(body));
			} catch (error) {
		...
		if (req.method === "POST" && rawPath.startsWith("/remote/api/")) {
			...
			const method = decodeURIComponent(rawPath.slice(12));
			if (!REMOTE_METHOD_ALLOWLIST.includes(method)) {
				json$1(res, 403, {
					ok: false,
					error: {
						code: "method-not-allowed",
						message: `方法不在白名单：${method}`
					}
				});
				return;
			}
	...
	const server = createServer((req, res) => {
		handle(req, res).catch((error) => {
			...
			if (!res.headersSent) json$1(res, 500, {
				ok: false,
				error: {
					code: "internal",
...
//#endregion
//#region src/host/admin-routes.ts
	...
	return ctx.webServer.register({
		...
		async handler(req, res) {
			...
			if (method === "GET" && pathname === "/remote-control/status") {
				...
				const urls = [...publicUrl !== "" ? [publicUrl] : [], ...configHost === "0.0.0.0" ? collectLocalHostnames().filter((name) => name.includes(".") && name !== "localhost").map((name) => `http://${name}:${gateway.port}`) : [`http://${configHost}:${gateway.port}`]];
				json(res, 200, {
					ok: true,
					status: {
			...
			if (method === "POST" && pathname === "/remote-control/pair-code") {
				...
				const urls = [...publicUrl !== "" ? [`${publicUrl}/?code=${issued.code}`] : [], ...configHost === "0.0.0.0" ? collectLocalHostnames().filter((name) => name.includes(".") && name !== "localhost").map((name) => `http://${name}:${gateway.port}/?code=${issued.code}`) : [`http://${configHost}:${gateway.port}/?code=${issued.code}`]];
				json(res, 200, {
					ok: true,
					code: issued.code,
...
const inject = ["apiProxy", "webServer"];
