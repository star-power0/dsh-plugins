/**
 * Aqua controls rendered below the master switch on the dedicated Glass theme
 * settings page: mode (mica / compatibility), blur/frost (mica mode only),
 * fluid color, background brightness, the backdrop source picker, and the
 * wallpaper picker with its two knobs. Every write goes straight through to
 * the layer, so the skin moves live. The whole control block renders nothing
...
export type AquaAppearanceRowComponentProps =
  PropsStore<ReturnType<typeof createAquaRowStore>>
...
export function AquaAppearanceRow(props: AquaAppearanceRowComponentProps) {
  ...
  // Off = the dedicated-page master switch is off: leave no trace below it.
