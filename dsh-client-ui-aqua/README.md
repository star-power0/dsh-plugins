...
Reload the web UI. Aqua is **on by default**; the master switch and all glass controls live in **Settings → Glass theme**. The page contains the mode, blur/frost (Mica mode), fluid color, background brightness, backdrop (fluid/wallpaper) with its wallpaper controls, and the particle-whale toggle. Turning the master switch off hides the control block.
