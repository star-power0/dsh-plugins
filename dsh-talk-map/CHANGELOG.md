...
- 起因：原 README 主体是上游英文原文（含上游安装/开发指引），本地维护副本缺少来源申明
  与本地改动说明。
- 改动：`README.md` / `README.zh.md` 重写为本地维护版——顶部「来源与许可」块（上游仓库、
  MIT、固定 commit `b2d36d6` = v0.1.0、上游 LICENSE 保留位置）+ 本地改动清单
  （`local.1` / `local.2` 卡片自动同步与阶梯摆位修复、双端接入方式）+ 功能说明
  （保留上游的功能描述——这些是插件自身能力）+ 使用表 + 数据位置 + 兼容性 + 开发 + License。
