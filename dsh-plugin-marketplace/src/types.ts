...
export interface MarketplaceInstalledEntry {
  ...
  enabled: boolean
  /** Whether the entry is activated through the Profile's cordis patch files. */
...
/** pickDirectory() outcome: null means the user cancelled the chooser. */
export interface MarketplacePickDirectory {
  path: string | null
}
