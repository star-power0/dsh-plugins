...
// src/host/index.ts
import { existsSync as existsSync4, readFileSync as readFileSync6, writeFileSync as writeFileSync2, mkdirSync as mkdirSync2, rmSync as rmSync2, renameSync as renameSync2, cpSync } from "node:fs";
...
function resolveStoreDir(dir, storeDir) {
  return isAbsolute(storeDir) || WINDOWS_ABSOLUTE_PATH.test(storeDir) ? storeDir : resolve2(dir, storeDir);
}
var JobTable = class {
...
import { existsSync as existsSync2, readFileSync as readFileSync4, readdirSync } from "node:fs";
...
function readSourceText(path, maxChars = 262144) {
  try {
    return readFileSync3(path, "utf8").slice(0, maxChars);
  } catch {
    return "";
  }
}
...
function packagePatchPath(packageName, dir) {
  ...
  try {
    ...
    return join2(dirname2(manifestPath), patch);
  } catch {
    return null;
  }
}
function packageEntryPath(packageName, dir) {
  ...
  try {
    ...
    return join2(dirname2(manifestPath), entry);
  } catch {
    return null;
  }
}
...
function profileLocation(ctx) {
  ...
  if (baseUrl !== void 0) {
    ...
    const dir = /\.(yml|yaml|json)$/.test(basename(raw)) ? dirname3(raw) : raw;
...
function packageManifestPath2(packageName, dir) {
  try {
    const require2 = createRequire2(join3(dir, "package.json"));
    return require2.resolve(packageName + "/package.json");
  } catch {
    return null;
  }
...
function exportsPatch(packageName, dir) {
  const path = packageManifestPath2(packageName, dir);
  if (path === null) return false;
  try {
    const manifest = JSON.parse(readFileSync4(path, "utf8"));
...
function patchActivatedPackages(dir) {
  const names = /* @__PURE__ */ new Set();
  for (const candidate of ["cordis.patch.yml", "cordis.yml"]) {
    try {
      for (const row of extractPatchRows(readFileSync4(join3(dir, candidate), "utf8"))) {
...
function installedEntries(manifest, dir, pluginDir) {
  ...
  for (const packageName of Object.keys(manifest.dependencies ?? {})) {
    ...
    entries.push({
      ...
      isBundle,
      linked: true,
      location: manifestPath === null ? "" : dirname3(manifestPath),
  ...
  const scanLocation = (location) => {
    try {
      const pluginManifest = JSON.parse(readFileSync4(join3(location, "package.json"), "utf8"));
  ...
  try {
    for (const item of readdirSync(pluginDir, { withFileTypes: true })) {
      ...
      const location = join3(pluginDir, item.name);
      scanLocation(location);
      if (item.name.startsWith("@")) {
        try {
          for (const child of readdirSync(location, { withFileTypes: true })) {
            if (!child.isDirectory()) continue;
            scanLocation(join3(location, child.name));
          }
        } catch {
        }
      }
...
function marketplaceSettingsPath() {
  const base = process.env.DSH_HOME?.trim() || join4(homedir(), ".dsh");
  return join4(base, "marketplace", "settings.json");
}
function readMarketplaceSettings() {
  try {
    const parsed = JSON.parse(readFileSync5(marketplaceSettingsPath(), "utf8"));
...
function defaultPluginRoot(profileDir) {
  return join4(resolve3(profileDir), "node_modules");
}
function defaultAgentWorkspaceDir() {
  return join4(dirname4(marketplaceSettingsPath()), "agent-workspace");
}
function agentWorkspaceLocation() {
  ...
  if (custom) {
    if (!existsSync3(workspaceDir) || !statSync(workspaceDir).isDirectory()) {
...
function persistAgentWorkspace(requestedDir) {
  ...
  if (value !== "") {
    ...
    if (!existsSync3(requested) || !statSync(requested).isDirectory()) {
...
function pluginTarget(profile, packageName) {
  return join4(profile.pluginDir, ...packageName.split("/"));
}
...
function isManagedPluginTarget(profile, packageName, target) {
  ...
  try {
    const manifest = JSON.parse(readFileSync5(join4(target, "package.json"), "utf8"));
...
function dependencyPath(root, packageName) {
  return join4(root, ...packageName.split("/"));
}
...
function linkProfilePeerDependencies(target, profileDir) {
  ...
  for (const packageName of Object.keys(peers)) {
    ...
    const destination = dependencyPath(join4(target, "node_modules"), packageName);
    ...
    const candidates = providedNames.flatMap((providedName) => [
      dependencyPath(join4(profileDir, "node_modules"), providedName),
      dependencyPath(join4(dirname4(profileDir), "node_modules"), providedName)
    ]);
...
function profilePackagePath(profileDir, packageName) {
  return join4(profileDir, "node_modules", ...packageName.split("/"));
}
...
// src/host/pick-directory.ts
import { spawn as spawn3 } from "node:child_process";
function windowsFolderDialogScript() {
  return [
...
async function pickWindowsDirectory(signal, internals = {}) {
  const spawner = internals.spawner ?? spawn3;
  return await new Promise((resolve4, reject) => {
    ...
    child.on("close", (code) => {
      finish(() => {
        ...
        resolve4(encoded === "" ? null : Buffer.from(encoded, "base64").toString("utf8"));
      });
    });
  });
}
async function pickDirectoryPath(capability, signal, internals = {}) {
  const platform = internals.platform ?? process.platform;
  if (platform === "win32") return await pickWindowsDirectory(signal, internals);
  if (capability !== void 0 && capability.kind === "native" && capability.pick !== void 0) {
...
var _restart_dec, _diagnoseConflicts_dec, _pickDirectory_dec, _setAgentWorkspaceDir_dec, _agentWorkspace_dec, _setInstallDir_dec, _installLocation_dec, _installed_dec, _jobStatus_dec, _setEnabled_dec, _uninstall_dec, _update_dec, _manualInstall_dec, _installPlugin_dec, _guidedTask_dec, _details_dec, _search_dec, _a, _init;
var MarketplaceService = class extends (_a = TypertRemoteService, _search_dec = [Remote("search")], _details_dec = [Remote("details")], _guidedTask_dec = [Remote("guidedTask")], _installPlugin_dec = [Remote("installPlugin")], _manualInstall_dec = [Remote("manualInstall")], _update_dec = [Remote("update")], _uninstall_dec = [Remote("uninstall")], _setEnabled_dec = [Remote("setEnabled")], _jobStatus_dec = [Remote("jobStatus")], _installed_dec = [Remote("installed")], _installLocation_dec = [Remote("installLocation")], _setInstallDir_dec = [Remote("setInstallDir")], _agentWorkspace_dec = [Remote("agentWorkspace")], _setAgentWorkspaceDir_dec = [Remote("setAgentWorkspaceDir")], _pickDirectory_dec = [Remote("pickDirectory")], _diagnoseConflicts_dec = [Remote("diagnoseConflicts")], _restart_dec = [Remote("restart")], _a) {
  ...
  async installed() {
    try {
      ...
      await Promise.all(entries.map(async (entry) => {
        if (entry.packageName === SELF_PACKAGE) {
  ...
  async pickDirectory() {
    try {
      const picker = this.ctx.directoryPicker;
      const path = await pickDirectoryPath(picker?.capability(), new AbortController().signal);
      return ok({ path });
    } catch (error) {
      return toFailure(error);
    }
  ...
  async startJob(kind, repo, ref) {
    ...
    try {
      ...
      if (directSelfUpdate) {
        return fail("self-update-disabled", "Marketplace self-update is disabled for this managed installation.");
      }
      ...
      if (kind === "update" && existsSync4(target)) {
        try {
          const targetManifest = JSON.parse(readFileSync6(join5(target, "package.json"), "utf8"));
  ...
  async driveInstall(job, profile, spec, before, beforeDeclaresBundle, requiresRestart = true, customTarget = null) {
    ...
    const stageDir = join5(dirname5(marketplaceSettingsPath()), "staging", job.jobId);
    ...
    try {
      ...
      const stagedManifest = packageManifestPath2(job.packageName, stageDir);
      ...
      cpSync(dirname5(stagedManifest), target, {
        recursive: true,
        dereference: true,
...
__decorateElement(_init, 1, "pickDirectory", _pickDirectory_dec, MarketplaceService);
...
__publicField(MarketplaceService, "inject", ["skills", "directoryPicker"]);
