window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function WallpaperPicker() {
		  ...
		  // Local patch: the picker lists only wallpapers this bundle can actually
		  // render (Video/Web). Scene and Application wallpapers need Wallpaper
		  // Engine's own renderer, so listing them as disabled rows only pads the
		  // dropdown — `hiddenCount` keeps them accounted for in the summary line.
		  const allWallpapers = sel.inventory.wallpapers;
		  const list = allWallpapers.filter(isRotatableWallpaper);
		  const hiddenCount = allWallpapers.length - list.length;
		    ...
		    React.createElement("div", { className: "we-picker__row" },
		      React.createElement("span", { className: "we-picker__hint" },
		        (group
		          ...
		          : list.length + " 个可用壁纸" + (hiddenCount > 0 ? " · 已隐藏 " + hiddenCount + " 个 Scene/应用类" : "")) +
		        (sel.rotationEnabled ? " · 自动轮转中" : "")),
		    ),
		  );
		}

		// ── Settings page shell (local patch) ───────────────────────────────────────
		// A framed viewport with a play glyph: a picture frame that moves. Drawn here
		// rather than reusing a stock settings glyph so the page is recognisable in the
		// settings rail at a glance.
		function WallpaperGlyph({ size = 34 }) {
		  return React.createElement("svg", {
		    width: size, height: size, viewBox: "0 0 24 24", fill: "none",
		    stroke: "currentColor", strokeWidth: 1.7, "aria-hidden": "true",
		  },
		    React.createElement("rect", {
		      x: 2.5, y: 4, width: 19, height: 14, rx: 2.2,
		      strokeLinejoin: "round",
		    }),
		    React.createElement("path", {
		      d: "M2.5 14.2l4.3-3.6 3.1 2.6", strokeLinecap: "round", strokeLinejoin: "round",
		    }),
		    React.createElement("path", {
		      d: "M10.6 15.8l3.4-4 3 3.3 1.9-1.6 2.6 2.2",
		      strokeLinecap: "round", strokeLinejoin: "round", opacity: 0.55,
		    }),
		    React.createElement("path", {
		      d: "M8.4 21.2h7.2", strokeLinecap: "round",
		    }),
		    React.createElement("circle", { cx: 16.4, cy: 8.6, r: 1.5, fill: "currentColor", stroke: "none" }),
		  );
		}

		function WallpaperSection() {
		  return React.createElement("div", { className: "we-page" },
		    React.createElement("div", { className: "we-page__hero" },
		      React.createElement("span", { className: "we-page__heroIcon" },
		        React.createElement(WallpaperGlyph, null)),
		      React.createElement("div", null,
		        React.createElement("div", { className: "we-page__title" }, "动态壁纸"),
		        React.createElement("div", { className: "we-page__meta" }, "Wallpaper Engine 壁纸作为对话背景"),
		      ),
		    ),
		    React.createElement("p", { className: "we-page__lead" },
		      "从本机 Wallpaper Engine 库中选择壁纸，渲染在对话界面后方。仅列出可在此渲染的 Video 与 Web 类型；Scene（3D）与应用类壁纸需要 Wallpaper Engine 自身的引擎，不在列表中。"),
		...
		const CSS = `
		  /* Settings page shell (local patch). */
		  .we-page { max-width: 760px; color: var(--dsw-alias-label-primary, #1f2329); }
		  .we-page__hero { display: flex; align-items: center; gap: 14px; margin-bottom: 12px; }
		  .we-page__heroIcon { flex: none; color: var(--dsw-alias-label-tertiary, #6b7280); display: flex; }
		  .we-page__title { font-size: 20px; font-weight: 650; letter-spacing: 0.2px; }
		  .we-page__meta { font-size: 14px; color: var(--dsw-alias-label-tertiary, #6b7280); }
		  .we-page__lead { font-size: 15px; line-height: 1.7; color: var(--dsw-alias-label-tertiary, #6b7280); margin: 0 0 18px; }
		...
		function apply(ctx) {
		  ...
		  // 2. Local patch: own settings page instead of a row crammed into General.
		  //    The picker carries a playlist editor and four sliders, which is far more
		  //    surface than a `settings.general.item` row is meant to hold.
		  if (ctx.slots) {
		    ctx.slots.inject("settings.section", () =>
		      ctx.slots.register(
		        { name: "settings.section", id: "wallpaper-engine", order: 110, label: () => "动态壁纸" },
		        WallpaperSection,
		      ),
		    );
		  }
