...
## 2026-09-19 · 修复：手机端会话「⋯」菜单被抽屉压住（⑨e4）

- 症状（丞相报告 + 截图）：手机抽屉里点会话行的「⋯」，菜单出现在**下面那一层**——被抽屉盖住/压暗，点菜单项没反应。
- 根因（chrome-devtools @9223 实测，390×844 移动仿真经 pocket 代理 3081）：
  - 抽屉被强制 `z-index: 1200`（为压过第三方插件的 1100/1050 遮罩，见抽屉规则的 z-index 注释），
    而官方 `_19372_` 弹层家族固定 `z-index: 1100`。
  - 从抽屉内部打开的弹层（会话行「⋯」= 重命名/分叉/归档/彻底删除/在新窗口打开）因此**低于自己的父抽屉**：
    菜单被抽屉与遮罩压暗，菜单项的点击落在抽屉表面 → 表现为「点了没用，只是把菜单关了」。
  - 附带发现：⑨e3 的注释声称覆盖六种容器变体，实际清单**漏了 `_portal_`**（本次一并补上）。
- 修复（`mobile.css.ts`，均在 `@media (max-width:1023px)` 内）：
  - **⑨e3**：补齐 `_portal_19372_` 到背景实底清单（六变体齐全）。
  - **⑨e4**（新增）：把弹层家族整体提到 `z-index: 1300 !important`，**高于抽屉的 1200**。
    不动抽屉的 1200——那是承载性数值（压第三方遮罩），降它会让抽屉重新被遮罩吃掉点击。
    1300 仍在固定横幅/Toast（z 9999）之下。
- 实测：菜单 `z=1300` > 抽屉 `z=1200`；5 个菜单项 hit-test 全部可点；真实点击「重命名」→ 菜单关闭 +
  行内重命名输入框出现并预填标题；「彻底删除」→ 弹出 `confirm` 确认框（删除流程走通）。
- 回归：手机端全量复测通过（样式注入、`.we-scrim` 去重、壁纸 DOM 已清、无横向溢出、
  可见半透明表面 0）；⑨e4 位于 `max-width:1023px` 块内，桌面端不受影响；产物 198,484 B，`node --check` 通过。
