> **本地定制版**。上游 [shaobeichen/dsh-pocket](https://github.com/shaobeichen/dsh-pocket)（GPL-2.0），npm `dsh-pocket@2.10.6`，2026-09-18 经 npmmirror 拉 tarball 安装（**非 git clone**）。
>
> **本机全部本地改动的唯一权威清单在 [LOCAL.md](./LOCAL.md)**：改动总表（ID / 文件 / 生效层级 / 上游是否覆盖）、手机窄屏定制明细、上游升级重打步骤、坑位速查。代码内以 `[DSH-LOCAL:<id>]` 注释标记，`grep -rn "DSH-LOCAL:" client/ lib/` 可一次找全。
>
> **本机环境速览**（详细步骤见 LOCAL.md）：手工注册双端（`package.json` 加 `link:` 依赖 + `cordis.patch.yml` insert `id: dsh-pocket`，`config.port: 3081`；**不进 bundles 栈**）；公网入口走本机 cloudflared 命名隧道（`05b67e61`）`pocket.starroute.me` → `http://127.0.0.1:3081`；手机访问入口：设置 →「手机访问」。运行时依赖 `qrcode` / `qrcode-terminal` 已在插件目录就地 `pnpm install --prod` 解析。配套 APK 壳工程在 `A:\DeepSeekHarness\pocket-apk\`（`build.ps1` 一键出包）。
>
> **改完如何生效**：`node client/build.mjs` 重建后，页面级改动（CSS/observer）刷新或重开 App 即可；`lib/index.js` / `lib/proxy.mjs` 属 host 级，**需重启 DSH Desktop**。任何 host 级编辑落刀后必须立刻 `node --check` + 真实启动探针（TDZ 事故教训，见 LOCAL.md §4）。
...
**改完想在本机先试？** 本插件在本机就是**本地定制版**（以 `link:` 接入），直接改源码即可：改 `client/` 后 `node client/build.mjs` 重建 → 刷新页面 / 重开 App 生效；改 `lib/` 属 host 级，需重启 DSH Desktop。完整改动清单、生效层级与升级重打步骤见 [LOCAL.md](./LOCAL.md)。
