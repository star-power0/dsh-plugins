...
function completePartialVisionResult(result, incomplete = false) {
  if (typeof result !== "object" || result === null || Array.isArray(result)) {
    return result;
  }
  const value = result;
  const string = (item) => typeof item === "string" ? item : "";
  const strings = (items) => Array.isArray(items) ? items.filter((item) => typeof item === "string") : [];
  const ocr = value.ocr && typeof value.ocr === "object" && !Array.isArray(value.ocr) ? value.ocr : {};
  const layout = value.layout && typeof value.layout === "object" && !Array.isArray(value.layout) ? value.layout : {};
  const semantics = value.semantics && typeof value.semantics === "object" && !Array.isArray(value.semantics) ? value.semantics : {};
  const visual = value.visual && typeof value.visual === "object" && !Array.isArray(value.visual) ? value.visual : {};
  return {
    summary: string(value.summary),
    ocr: {
      full_text: string(ocr.full_text),
      lines: Array.isArray(ocr.lines) ? ocr.lines.filter((line) => line && typeof line === "object" && typeof line.text === "string").map((line) => ({ text: line.text, ...(typeof line.language === "string" ? { language: line.language } : {}) })) : []
    },
    layout: {
      regions: Array.isArray(layout.regions) ? layout.regions.filter((region) => region && typeof region === "object").map((region, index) => ({ type: string(region.type), reading_order: Number.isFinite(region.reading_order) ? region.reading_order : index + 1, text: string(region.text) })) : []
    },
    semantics: {
      scene: string(semantics.scene),
      ...(typeof semantics.intent === "string" ? { intent: semantics.intent } : {}),
      entities: Array.isArray(semantics.entities) ? semantics.entities.filter((entity) => entity && typeof entity === "object").map((entity) => ({ name: string(entity.name), type: string(entity.type), ...(typeof entity.evidence === "string" ? { evidence: entity.evidence } : {}) })) : [],
      ...(Array.isArray(semantics.relations) ? { relations: semantics.relations.filter((relation) => relation && typeof relation === "object").map((relation) => ({ subject: string(relation.subject), predicate: string(relation.predicate), object: string(relation.object) })) } : {})
    },
    visual: {
      ...(Array.isArray(visual.dominant_colors) ? { dominant_colors: strings(visual.dominant_colors) } : {}),
      ...(typeof visual.style === "string" ? { style: visual.style } : {}),
      ...(Array.isArray(visual.notes) ? { notes: strings(visual.notes) } : {})
    },
    uncertainty: [
      ...strings(value.uncertainty),
      ...(incomplete ? ["The vision provider omitted one or more structured evidence fields."] : [])
    ]
  };
}
...
${JSON_TEMPLATE_INSTRUCTION}`;
  ...
  const recoveredText = typeof text === "string" ? text.trim() : "";
  const recovered = rawResult === null && recoveredText !== "" && (finishReason === void 0 || finishReason === "stop");
  if (rawResult === null && !recovered) {
    const advice = finishReason === "length" ? `The answer was cut off (finish_reason=length). Raise the limit, e.g. modlens config set openai.extraBody '{"max_tokens":4096}'.` : `The gateway ended the answer with finish_reason=${quote(finishReason, (t) => truncate(t, 80))}, so it may be incomplete for a reason of its own. Check what that reason means for this endpoint before changing the request.`;
    throw new Error(
      `OpenAI-compatible API returned non-JSON output. ${advice} Output ended with: ${quote(text, tail)}`
    );
  }
  const candidate = rawResult ?? { summary: recoveredText };
  const rawMissing = missingSchemaFields(normalizeVisionResult(candidate));
  const result = completePartialVisionResult(normalizeVisionResult(candidate), rawMissing.length > 0);
...
async function analyzeImage(options) {
  ...
  if (chain.length === 1) {
    if (!options.provider && !options.providerBin && lastError instanceof Error) {
      ...
      if (hint) {
        // lastError can be a DOMException (e.g. an aborted fetch) whose
        // `message` is a getter-only accessor; in ESM strict mode `+=` throws
        // "Cannot set property message ... only a getter" and masks the real
        // vision failure. Append in place when writable, otherwise wrap.
        const own = Object.getOwnPropertyDescriptor(lastError, "message");
        if (own?.writable) {
          lastError.message += hint;
        } else {
          lastError = new Error(`${lastError.message ?? String(lastError)}${hint}`, { cause: lastError });
        }
      }
    }
    throw lastError;
