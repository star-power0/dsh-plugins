...
| 想再看别的？再打一段字问一遍 | 界面里就有「刷新」「切换视图」按钮，点一下，模型更新数据 |
...
- **唯一通道**：```dsh-ui 围栏是唯一的 UI 输出方式，组件渲染在回答正文里（本地 v0.8.9 起已移除 `render_ui` 工具与会话面板，`panel`/`append` 字段无效）
- **围栏校验**：`validate_dsh_ui` 工具在发出复杂围栏（≥3 节点或含表格）前做 JSON 预检，可修复的错误直接返回修好的 JSON
