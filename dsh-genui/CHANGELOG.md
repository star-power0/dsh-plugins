...
### 验证
- `tsdown` 重建 `lib/client.js`（确定性构建，产物含 `!'panel'.startsWith(n) ? [] : [...]` 过滤）；`plugin-safety check` 16 插件双端全绿，无 id/serverName 冲突；改动前存档 `plugin-snapshots/20260823-161021-fix-genui-slash-filter`。桌面端同样加载同一 client bundle，双端无需分改动。
