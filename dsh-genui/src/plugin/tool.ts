 ...
 */
const FIELD_HINTS =
  '字段名速查（容器子项不一定叫 items）：① steps 的子项字段是 "steps"（不是 "items"），子项用 title/desc ② table 的列名字段是 "columns"（不是 "headers"），行字段是 "rows" ③ keyvalue 的字段是 "pairs"（[{key,value}]）④ callout 的语气字段是 "tone"（info/success/warning/error，不是 level）⑤ list/tabs/accordion/grid/card/col/row 才用 "items" ⑥ quiz 用 question+options，plot 用 series，chart 用 data 或 series'
...
export function createValidateDshUiTool(): ToolDefinition {
  return {
    ...
    async execute(args: unknown): Promise<JsonValue> {
      ...
      // A spec whose EVERY node was dropped by the repair pass renders as an
      // empty banner-only block — the exact "title shows, content gone"
      // failure class. Refuse it with the field-name cheat sheet; a partial
      // drop stays ✅ but carries a warning so the model can self-correct
      // before emitting (the renderer drops such nodes silently by design).
      const rawCount = countGenuiNodes(parsed, GENUI_LIMITS.maxNodes)
      const dropped = rawCount - count
      if (count === 0) {
        return `❌ spec 里没有任何可渲染组件 —— 全部 ${rawCount} 个节点都因字段/结构错误被修复层丢弃（渲染出来只会剩一个空标题）。最常见原因是字段名写错：\n${FIELD_HINTS}\n请按上面的字段名修正后重新调用本工具验证。`
      }
      if (dropped > 0) {
        return `⚠️ spec 可用（${count} 个组件），但 ${dropped} 个节点会因字段/结构错误被静默丢弃——发出去的内容会缺块。最常见原因是字段名写错：\n${FIELD_HINTS}\n修正字段后重新验证；确认剩余内容已完整时也可直接发出围栏。`
      }
