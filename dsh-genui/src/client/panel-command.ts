...
export function createPanelSlashSource(sendInstruction: (sessionId: SessionId, instruction: string) => void): InputTriggerSource {
  return {
    ...
    candidates: async (_session, req) => {
      // Only surface the /panel candidate when the user's typed query
      // could still be aiming at it: empty (bare `/`) or a prefix of
      // "panel". Without this filter the candidate would shadow other
      // sources and show up first for unrelated commands like /compact.
